...
public class KinesisConsumerService {
    ...
    private static InitialPositionInStream getEnvVal(String position, InitialPositionInStream defaultValue) {
        if (System.getenv(position) == null) {
            return defaultValue;
        }
        switch (System.getenv(position)) {
            case "TRIM_HORIZON":
    ...
    public static void main(String[] args) {
        ...
        new KinesisConsumerService(streamName, region).run();
    }
    ...
    private KinesisConsumerService(String streamName, String region) {
    ...
    private void run() {
         ...
         */
        final InitialPositionInStream initialPosition = getEnvVal("INITIAL_POSITION", InitialPositionInStream.TRIM_HORIZON);
        InitialPositionInStreamExtended initialPositionStream = null;

        if (initialPosition == InitialPositionInStream.AT_TIMESTAMP) {
            String timestamp = getEnvVal("TIMESTAMP", "20210101000000");
            SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddhhmmss");
            sdf.setTimeZone(TimeZone.getTimeZone("UTC"));
            try {
                initialPositionStream = InitialPositionInStreamExtended.newInitialPositionAtTimestamp(sdf.parse(timestamp));
            } catch (java.text.ParseException e) {
                throw new IllegalArgumentException("Cannot parse dateformat (yyyyMMddhhmmss) : " + timestamp);
            }
        } else {
            initialPositionStream = InitialPositionInStreamExtended.newInitialPosition(initialPosition);
        }

        Scheduler scheduler = new Scheduler(
                configsBuilder.checkpointConfig(),
                configsBuilder.coordinatorConfig(),
                configsBuilder.leaseManagementConfig().initialPositionInStream(initialPositionStream),
